...
void assemble(const char * file_name, instruction* instructions, int num_instructions){
	...
	strcpy(hack_file, ".hack");
	FILE *file;
	file = fopen(hack_file, "w");

	for (int i = 0; i < num_instructions; i++){
		opcode opcode;

		if (instructions[i].a_c == A_type){

			if ()

		}

		else if (instructions[i].a_c == C_type){
			opcode = instruction_to_opcode(instructions->instruction.c_instruction);
		}
